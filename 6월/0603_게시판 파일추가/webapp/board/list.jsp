...
<html>
  ...
  <body>
    ...
    <script>
          ...
          let record_total_count = ${record_total_count}; // 전체 글의 갯수
          let record_count_per_page = ${record_count_per_page}; // 한 페이지 당 보여질 게시글 갯수
          let navi_count_per_page = ${navi_count_per_page}; // 네비게이터 갯수
      ...
      if(record_total_count % record_count_per_page > 0) {
         ...
         // 네비게이터 시작 값
         let startNavi = Math.floor((cpage - 1) / record_count_per_page) * record_count_per_page + 1;


      // 네비게이터 마지막 값
      let endNavi = startNavi + navi_count_per_page - 1;

      if(endNavi > pageTotalCount) { // 전체 페이지수 보다 endNavi의 수가 클 수는 없다.
      ...
      if(startNavi == 1) {needPrev = false;} // 이전 네비게이터가 필요 없을 경우 false
      if(endNavi == pageTotalCount) {needNext = false;} // 전체 페이지 수와 같을 경우 false
