...
//        model.addAttribute("record_total_count", bdao.getBoardCount());
	...
	public String search(String writer, String title, Model model) throws Exception {
		
		List<BoardDTO> list;
		
		if (writer.trim().isEmpty() && title.trim().isEmpty()) {
			list = bserv.selectAll(1, 10); // 예시로 1페이지, 10개씩 가져오는 방식
		} else {
			list = bserv.search(writer, title);
		}
		
		 // 페이지네이션 정보 재계산
	    int cpage = 1; // 예시로 1페이지
	    int recordsPerPage = 10; // 예시로 10개씩 출력
	    int totalRecords = bserv.getBoardCount(); // 전체 데이터 개수 (검색 결과의 전체 개수)
	    int naviCountPerPage = 5; // 페이지네이션에서 보여질 페이지 수
	    ...
	    model.addAttribute("record_total_count",  totalRecords); // 전체 데이터 개수
	    model.addAttribute("record_count_per_page", recordsPerPage); // 페이지당 데이터 개수
	    model.addAttribute("navi_count_per_page", naviCountPerPage); // 페이지네이션에서 보여질 페이지 수
